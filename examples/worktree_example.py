...
from omni.git.worktree import WorktreeManager, WorktreeEnv
...
async def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  Created feature.py in worktree")
        ...
        async with WorktreeEnv(manager, "feature-b") as env:
            if env:
                ...
                print(f"  Created utils.py in worktree")
            else:
                print(f"  Worktree creation failed (fallback to main repo)")
        
        # Worktree is automatically cleaned up
        print(f"  Worktree cleaned up after task completion")
        ...
        if task1_info:
            ...
            if success:
                print(f"  Successfully merged task-1 changes to main")
                # Check that file exists in main repo
                assert (repo_path / "task-1.txt").exists()
                print(f"  File task-1.txt now in main repo")
            else:
                print(f"  Merge failed (conflict)")
