...
class ParallelExecutionEngine:
    ...
    async def resume(
        ...
        # Load task states
        task_states = db.load_task_states(execution_id)
    ...
    def _create_task_executor(self) -> Callable[[Task], Awaitable[Any]]:
        ...
        async def execute_task(task: Task) -> Any:
            ...
            try:
                ...
                raise TimeoutError(f"Task {task.task_id} timed out after {self.config.timeout_per_task}s")
    ...
    def _handle_task_complete(
        ...
        if (status == TaskStatus.FAILED and 
            ...
            self.completed_at = datetime.now()
            partial_status = self._determine_final_status()
            self._update_metrics()
