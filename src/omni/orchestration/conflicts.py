...
class FileConflict:
    ...
    task_contents: dict[str, str] = field(
        default_factory=dict
    )  # task_id -> modified content
...
class Resolution:
    ...
    requires_sequential: bool = (
        False  # Whether tasks need to be re-executed sequentially
    )
...
class ConflictResolver:
    ...
    def __init__(self, llm_merge_threshold: int = 3):
        ...
        logger.info(
            f"ConflictResolver initialized (llm_merge_threshold={llm_merge_threshold})"
        )
    ...
    def detect_conflicts(self, results: list[TaskResult]) -> list[FileConflict]:
        ...
        for result in results:
            ...
            for file_path in all_files:
                ...
                file_content = None
                if (
                    "file_contents" in result.outputs
                    and file_path in result.outputs["file_contents"]
                ):
                ...
                file_to_tasks[file_path].append(
                    (
                        result.task_id,
                        {
    ...
    def _classify_conflict(
        self, file_path: str, task_data: list[tuple[str, dict[str, Any]]]
    ) -> ConflictType:
        """
        ...
        if not all_have_content:
            ...
            logger.warning(
                f"Cannot analyze conflict for {file_path}: missing file content"
            )
    ...
    def _reconstruct_file_content(
        self, file_path: str, applied_edits: list[dict[str, Any]]
    ) -> str | None:
        """
    ...
    def resolve(
        self, conflict: FileConflict, original_content: str | None = None
    ) -> Resolution:
        """
