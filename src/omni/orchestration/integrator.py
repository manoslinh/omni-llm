...
class ResultIntegrator:
    ...
    def integrate(
        ...
        """
        logger.info(f"Integrating {len(results)} task results for goal: {original_goal}")
    ...
    def _extract_file_changes(self, results: list[TaskResult]) -> dict[str, dict[str, Any]]:
        """
        ...
        for result in results:
            ...
            for file_path in set(files_modified + files_created):
                ...
                if file_path in file_contents:
                    file_changes[file_path]["contents"][result.task_id] = file_contents[file_path]
    ...
    def _resolve_conflicts(
        ...
        for file_path, change_data in file_changes.items():
            if file_path in conflicted_files:
                ...
                conflict = next(
                    (c for c in conflicts if c.file_path == file_path),
                    None
                )
                ...
                if conflict:
                    ...
                    logger.warning(f"File {file_path} marked as conflicted but no conflict object found")
            else:
    ...
    def _generate_commit_message(
        ...
        if successful_results:
            lines.append(f"Achieve: {original_goal[:50]}{'...' if len(original_goal) > 50 else ''}")
