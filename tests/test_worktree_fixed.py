...
class TestWorktreeManager:
    ...
    async def test_create_worktree(self, manager):
        ...
        info = await manager.create("task-001", base_branch="master")
    ...
    async def test_create_max_limit(self, manager):
        ...
        with pytest.raises(WorktreeError, match="Max worktrees limit reached"):
    ...
    @pytest.mark.asyncio
    async def test_remove_nonexistent_idempotent(self, manager):
        """Test removing nonexistent worktree is idempotent (no error)."""
        # Should not raise
    ...
    async def test_remove_idempotent(self, manager):
        ...
        await manager.create("task-001")
        await manager.remove("task-001")
        # Second remove should NOT raise (idempotent)
        await manager.remove("task-001")  # Should not raise
    ...
    async def test_get_diff(self, manager):
        ...
        worktree_repo = GitRepository(path=str(info.path))
        await worktree_repo.commit(["new.txt"], "Add new file", ai_attributed=True)
...
class TestWorktreeEnv:
    ...
    async def test_aexit_force_fallback(self, manager, monkeypatch):
        ...
        # Don't create worktree first - WorktreeEnv will create it
